...
const UpdateEmployee = () => {
  ...
  const [age, setAge] = useState(employee.employeeAge.toString());
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(process.env.UpdateEmployee_API_URL, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        // onGoBack(); // Call the callback to refresh the list in GetEmployees
        // navigation.goBack(); // Navigate back to the previous screen
        navigation.navigate("GetEmployees", { refresh: true });
      } else {
        const errorText = await response.text();
        console.error("Failed to update employee:", errorText);
        showAlert("Failed to update employee. Please try again later.");
      }
    } catch (error) {
